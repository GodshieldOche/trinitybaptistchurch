...
import { getConferenceDetails, postConferenceSermons, postUpdateConferenceSermons } from '../../../../redux/features/conference';
...
const Adder = ({ name }) => {
    ...
    useEffect(() => {
        ...
        if (name === 'conference sermon Details') {
            dispatch(getConferenceDetails(id)).then(result => {
                if (!result.error) {
                    const { title, category, topic, preacher, book, chapter, verse,
                        date, description, imageUrl, audioUrl, youtubeLink } = result.payload.conference.sermons[index]
                    setTitle(title)
                    setTopic(topic)
    ...
    const handleSubmit = (e) => {
        ...
        if (title && topic && category && date && preacherName && book && chapter && verse && description && audioUrl) {
            ...
            name === "add conference sermon" && dispatch(postConferenceSermons({
            ...
            if (name === "conference sermon Details") {
                const sermon = conference.sermons[index]
                dispatch(postUpdateConferenceSermons({
